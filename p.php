                            ...
                            <canvas id="radarChart">
                                ...
                                if ($result->num_rows > 0) {
                                    ...
                                    while ($row = $result->fetch_assoc()) {
                                        // Mendapatkan teks dari $row["INDIKATOR"]
                                        $indikatorText = $row["INDIKATOR"];

                                        // Membagi teks menjadi array kata-kata
                                        $words = str_word_count($indikatorText, 1);

                                        // Mengambil hanya 5 kata pertama
                                        $limitedWords = array_slice($words, 0, 5);

                                        // Menggabungkan array kata-kata menjadi teks kembali
                                        $limitedText = implode(' ', $limitedWords);

                                        // Menambahkan label yang sudah dimodifikasi ke dalam $labels[]
                                        $labels[] = $row['ID_INDIKATOR'] . ', ' . $limitedText;

                                        // Menambahkan data dan borderColors seperti biasa
                                        $data[] = $row["totalaudit"];
